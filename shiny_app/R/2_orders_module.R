...
orders_module <- function(input, output, session, vendor_info){
  ...
  observeEvent(input$resolve_awaiting_orders, {

    awaiting_order_choices <- awaiting_orders()$order_uid
    names(awaiting_order_choices) <- paste0("Order #", awaiting_orders()$order_number)

    showModal(
      modalDialog(
        title = "Reviewing Orders for Acceptance:",
        size = "s",
        footer = list(
        ...
        fluidRow(
          column(
            12,
            span(
              shinyWidgets::pickerInput(session$ns("select_order_for_review"),
                                        icon_text("search", "Select Order to Review:"),
                                        choices = awaiting_order_choices,
                                        selected = awaiting_order_choices[1]),

              actionButton(session$ns("accept_order"),
                           "Accept Order",
                           class = "btn btn-success"),
              actionButton(session$ns("reject_order"),
                           "Reject Order",
            ...
            textAreaInput(session$ns("vendor_response_text"),
                          "Response Comments:",
                          placeholder = "Add details for why you are accepting or rejecting the order here:"
            )
          )
        )
      )
...
orders_prep <- reactive({
  req(orders(), completed_orders(), awaiting_orders())
