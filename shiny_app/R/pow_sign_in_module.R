...
#'
#'
pow_sign_in_module_2_ui <- function(id) {
  ns <- shiny::NS(id)
  ...
  register_passwords <- div(
    ...
    div(
      ...
      shiny::checkboxInput(
        ns("tos_pp_agree"),
        label = HTML("By checking this box you agree to our <a href='https://www.powwater.com/terms-conditions-vendors/' target='blank_'>Terms of Service</a> and <a href='https://www.powwater.com/privacy/' target='blank_'>Privacy Policy</a>"),
        value = FALSE
      )
    ),
    div(
...
#' @export
#'
pow_sign_in_module_2 <- function(input, output, session) {
  ns <- session$ns
  ...
  shiny::observeEvent(input$submit_continue_sign_in, {
    ...
    tryCatch({
      ...
      invite <- polished:::.global_sessions$get_invite_by_email(email)
  ...
  observeEvent(input$tos_pp_agree, {

    if (isTRUE(input$tos_pp_agree)) {
      enable("register_submit")
    } else {
      disable("register_submit")
    }
  ...
  observeEvent(input$register_js, {
    ...
    if (isTRUE(is_email) && !polished:::is_valid_email(hold_email)) {
    ...
    tryCatch({
      polished:::.global_sessions$register_email(
        hold_email,
        hold_password,
        hashed_cookie
      )
  ...
  check_jwt_email_valid <- reactive({
    ...
    is_email <- is.null(input$check_jwt$jwt)
    if (isTRUE(is_email) && !polished:::is_valid_email(isolate({input$sign_in_email}))) {
